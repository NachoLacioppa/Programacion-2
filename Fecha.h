...
class Fecha
{
  ...
  public:
    ...
    bool cargar()
    {
        ...
        cout << "A\245O: ";
        cin >> anio;
        if(anio < 1900 || anio > 2090)
        {
        fechaInvalida();
        return false;
        }
        valido=cantDiasMes(dia, mes);
        if (valido==-1)
        {
            cout << " FECHA INVALIDA. NO ES A/245O BISIESTO" << endl;
            return false;
        }
        return true;
    }
...
void fechaInvalida()
{
    clrscr();
    titulo_programa();
    cout<<" FECHA INVALIDA \a\a\a"<<endl;
    pause();
    return;
}
...
int Fecha::cantDiasMes(int dia, int posMes)
{
  int vmes[]={31,28,31,30,31,30,31,31,30,31,30,31};
  if(esbisiesto(anio)) vmes[1]=29;
  if (dia<=0 || dia>vmes[posMes-1]){return -1;}
  if (posMes<1 || posMes>12) {return -1;}
  return vmes[mes-1];
  }
